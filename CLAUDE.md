...
| Local-scan override | `ALLOW_LOCAL_SCAN=1` env var (dev only) skips both SSRF layers and sets Playwright `ignoreHTTPSErrors` (self-signed certs) — for scanning localhost/VPN targets | Unset by default; never set in a deployed backend |
| Rate limiting | `express-rate-limit` on `POST /api/scan` | 10 req/min, configurable |
| CSP | Content-Security-Policy meta tag in `index.html` | `default-src 'self'`, allows Google Fonts; AI calls go through backend. No `frame-ancestors` (ignored in meta tags) — add as HTTP header in hosting if ever deployed publicly |
...
- `backend/.env` — loaded by `dotenv/config` (first import in `server.ts`). `PORT` (default 3001), `FRONTEND_URL`, `PAGE_TIMEOUT`, `DB_PATH`, `LOG_LEVEL`, `API_KEY`, `OPENROUTER_API_KEY`, `ALLOW_LOCAL_SCAN`. Real environment variables win over `.env`. Note: Node 20.6+ `--env-file` is not used — the project targets Node 18.
