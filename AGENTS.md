...
- **T26** (CSP + optional Groq proxy) — CSP meta tag added; the backend proxy for Groq calls was waived (backend proxy implemented post-audit on feat/groq-proxy).
...
| `src/components/Settings/Settings.tsx` | Groq LLM config (server AI status + model: 5 models available) |
