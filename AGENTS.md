...
Supports interactive multi-step flows (login, search, add to cart) via JSON step definitions or Playwright codegen script import. Includes visual regression detection comparing screenshots between scans of the same URL using pixelmatch.
...
| `src/api/routes/scan.ts` | POST scan (accepts flow/flowId/visualDiffThreshold), GET status |
...
| File | Purpose |
|------|---------|
| `src/pages/Home.tsx` | URL input + flow selector + scan progress polling |
...
| `src/services/api.ts` | Axios client → backend port 3001 (scan, reports, flows, baseline) |
